...
13. **Botón "Donar con MercadoPago" roto en `/donar`** — Emanuel probó el link y da error. El `href` (`https://www.mercadopago.com.ar/subscriptions/checkout?preapproval_plan_id=2c9380848e498e81018e5c7c5e4d0640`) es el mismo que traía la página original de WordPress desde la migración (Fase 2), sin ningún cambio de código desde entonces — no hay lógica nuestra en el medio, es un link estático directo a MercadoPago. El problema está del lado de la cuenta de MercadoPago, probablemente el plan de suscripción esté pausado/eliminado. **Necesito que Emanuel** entre a su cuenta de MercadoPago (Tu negocio → Suscripciones), verifique el plan y, si hace falta, cree uno nuevo y me pase el link de checkout actualizado — cambio de una sola línea en `src/app/donar/page.tsx` en cuanto lo tenga.
