...
10. **Cutover DNS** (Fase 5 del plan maestro) — ✅ **EJECUTADO Y VERIFICADO (27-ago-2026, 01:35 UTC)**. **El sitio nuevo está en vivo para el público**, en `www.usinadejusticia.org.ar` y en el dominio pelado; WordPress vive en `wp.usinadejusticia.org.ar`. Verificación completa en `docs/CUTOVER.md`: DNS coincidente en cuatro resolutores, los dos dominios asignados al proyecto de Vercel con certificado emitido, el pelado dando `308` al canónico conservando la ruta, las 8 páginas principales en `200`, las URLs viejas con fecha redirigiendo a `/noticias/:slug`, el 404 propio funcionando, `sitemap.xml` y `robots.txt` en `200`, la API de WordPress en `200` y `wp-admin` entrando al login dentro de `wp.`. En `/noticias`, **144 imágenes apuntando a `wp.` y cero al host viejo** — se confirmó así la predicción de la fase B: las imágenes destacadas no viven en `post_content` sino que WordPress las arma desde `siteurl`, y los `define` de `WP_HOME`/`WP_SITEURL` las normalizaron solas. **Fase E, en curso:** ✅ webhook de publicación instantánea funcionando (ver punto 11); ✅ **Google Search Console (27-ago)** — propiedad de tipo *Dominio* verificada por registro `TXT`, que cubre de una sola vez `www`, el dominio pelado y `wp.` (esto último sirve para vigilar cómo el sitio viejo desaparece del índice). Sitemap enviado y procesado: **877 páginas descubiertas**, consistente con los 842 posts más las páginas propias. Cuidado a tener presente si alguna vez hay que rehacerlo: la zona **ya tenía un `TXT` en `@`** con el SPF de Google Workspace — el de verificación se agrega como registro **nuevo**, nunca editando el existente, o se rompe el correo saliente. Verificado después del cambio que los dos conviven y resuelven. ✅ **Formulario de contacto reprobado desde el dominio real (27-ago)** — llega a destino, no cae en spam; el remitente `contacto@usinadejusticia.org.ar` y el DKIM `resend._domainkey` sobrevivieron intactos al cutover. ✅ **Credenciales de agente revocadas (27-ago)** — el usuario `agente-migracion` (rol editor, creado para la reasignación de categorías de la Fase 2) tenía viva la Application Password `claude-fase2b`, creada el 13-jul y sin usarse desde ese mismo día. Se anuló la contraseña y se borró el usuario, que no tenía ninguna entrada a su nombre. Nota para el futuro: el sitio **lee** de WordPress sin autenticarse —las notas publicadas son públicas—, así que esa llave solo servía para escribir y su revocación no afecta nada. También se rotó `REVALIDATE_SECRET` al configurar el webhook, así que la clave anterior ya no sirve. Se dejan a propósito, hasta terminar la lista, el acceso SSH y la sesión del CLI de Vercel en la máquina de Emanuel. Quedan Google Business Profile, endurecer HSTS y subir el TTL. **Registro de cómo se llegó hasta acá, que sigue siendo útil:** Runbook completo, con verificación y vuelta atrás en cada paso: **`docs/CUTOVER.md`**. Lo que se descubrió al planificarlo y que no estaba contemplado antes: el sitio nuevo le pide el contenido a WordPress **en el mismo dominio** que va a pasar a servir el sitio nuevo, así que mover el DNS sin más lo dejaría sin fuente de datos; y **215 de los 842 posts** tienen imágenes con URL absoluta al dominio actual escrita dentro del cuerpo del post (más los 5 PDFs de `/nosotros/transparencia` y el `/wp-admin` del equipo). Por eso WordPress tiene que mudarse antes a un subdominio. **Ya resuelto en código, de forma inerte**: una sola variable `WP_HOST` de la que se derivan la URL de la API (`src/lib/wordpress.ts`), el optimizador de imágenes, la CSP y los redirects (`next.config.mjs`), más una regla `/wp-content/:path*` que cubre de un saque los 215 posts, los PDFs y cualquier archivo no inventariado, y redirects de `/wp-admin` y `/wp-login.php`. Verificado ejecutando la config con y sin la variable: **sin `WP_HOST` la salida es idéntica a la de antes**, así que el cutover es un cambio de configuración reversible en un minuto, no un cambio de código bajo presión. Ejecución pendiente de Emanuel (hosting, DNS y registrador — el agente no tiene acceso).

    **EN EJECUCIÓN (26-ago). Fases A y B cerradas y verificadas:**
    - **A2 — subdominio.** `wp.usinadejusticia.org.ar` creado en hPanel apuntando al mismo directorio que WordPress. Resuelve a `147.93.37.235`, la IP directa del servidor: **el subdominio no pasa por el CDN de Hostinger**, a diferencia de `www` y del apex (ver más abajo). Sirve la REST API y `/wp-content/` con SSL propio.
    - **A/B4-5 — reescritura de URLs en la base.** Ejecutada por WP-CLI vía SSH (agente externo, supervisado): backup de 452 MB fuera de `public_html` antes de tocar nada, `--dry-run` antes de cada escritura, `--skip-columns=guid` en todas las corridas (el `guid` es el identificador único de RSS: cambiarlo haría que los 842 posts aparezcan como nuevos para todos los suscriptores) y alcance limitado a `wp_posts` — **`wp_postmeta` se dejó intacto a propósito**, porque guarda el JSON de Elementor con barras escapadas y un search-replace ahí lo corrompe; ese hueco lo cubre el redirect `/wp-content/:path*`. Total: **5.734 reemplazos**, incluida una variante con `http://` que no estaba en el plan original y apareció en el scrub final (5.189 filas). Verificado después: cero ocurrencias del host viejo en `post_content`, y la REST API devolviendo URLs nuevas.
    - **Los 4.407 "links internos" que quedaron sin migrar no son un problema** — se midieron sobre el contenido renderizado de los 842 posts: son solo **162 links distintos**, y 159 son del formato `/YYYY/MM/DD/slug`, que el propio sitio nuevo ya redirige (308) a `/noticias/slug`. Probado contra el servidor construido, no deducido. Los 3 restantes son páginas de adjunto sin valor. **No hace falta ninguna operación más sobre la base.**
    - **B1-B2 — `WP_HOST` en Vercel.** Cargada en Production y Preview (`wp.usinadejusticia.org.ar`) y redesplegada. Verificado en la cabecera `Content-Security-Policy` de producción: `wp.usinadejusticia.org.ar` presente en `img-src`, `media-src` y `connect-src`, **y el host viejo conviviendo en las tres** — es el diseño buscado (`LEGACY_WP_HOST` fijo en `next.config.mjs`), para que ningún paso intermedio del cutover pueda romper las imágenes.
    - **Hallazgo que conviene no confundir con un error:** después de la reescritura, las imágenes destacadas de las noticias **siguen sirviéndose desde el host viejo**. Es correcto. La imagen destacada no vive en `post_content` — WordPress la arma a partir de la opción `siteurl`, que sigue apuntando al dominio actual y **tiene que seguir así hasta la fase D**. Se normaliza sola cuando se agreguen los `define` de `WP_HOME`/`WP_SITEURL`.
    - **A3 — `www` adjuntado en Vercel (26-ago).** Vía CLI, sin tocar DNS ni el repo. No pidió TXT de verificación. El registro que pide es **`A` · `www` → `76.76.21.21`** (valor literal de `vercel domains inspect`, no una suposición). El dominio pelado se deja para la fase D a propósito.
    - **Descartada la opción de delegar los nameservers a Vercel**, que Vercel ofrece como alternativa. La zona tiene **5 MX de Google Workspace y un SPF `include:_spf.google.com`** (verificado por consulta DNS): delegar la zona dejaría a la organización **sin correo** —incluida `info@usinadejusticia.org.ar`, destino del formulario de contacto— y además se llevaría puesto el subdominio `wp.`, o sea la fuente de contenido del sitio nuevo. Se usa el registro A puntual, que deja el resto de la zona intacta.
    - **Cambio de plan pedido por Emanuel (26-ago): el apex se mueve el mismo día que `www`, no en una fase posterior.** El motivo es correcto y no lo había ponderado bien: **casi nadie escribe el "www"**, así que mover solo `www` habría dejado a la mayor parte del tráfico real viendo el sitio viejo. Las fases C y D se fusionaron en el runbook. El redirect del apex al canónico se resolvió **en código** (`next.config.mjs`) y no con la opción del panel de Vercel, para poder verificarlo **antes** del cutover en vez de descubrir un error con el DNS ya movido. Probado con build real y distintas cabeceras `Host`: apex → 308 al canónico conservando la ruta, `www` → 200 sin redirigir, subdominio cualquiera → sin match. Ese último caso importa: Next pasa el valor del matcher a una regex **sin anclarla**, así que sin `^...$` el apex habría matcheado como substring dentro del propio canónico y el sitio se habría redirigido a sí mismo en bucle.
    - **El DNS queda como paso manual: la API de Hostinger no está habilitada en esta cuenta** (verificado por Emanuel en `hpanel.hostinger.com/api`; probablemente una limitación del plan). Se evaluó porque Emanuel pidió que todo lo automatizable lo haga un agente, y la API sí tiene lo necesario (`GET` de la zona como backup, `POST .../validate` como dry-run, `DELETE` por `name`+`type`, `PUT` para crear) — queda documentado en `docs/CUTOVER.md` por si el plan cambia, con dos prohibiciones: `overwrite: true` (reemplaza la zona entera: se llevaría los MX de Google, el SPF y `wp.`) y el endpoint `.../reset`. **Tampoco hay una segunda vía por SSH, y no es cuestión de permisos**: el SSH da una terminal dentro del servidor donde vive WordPress, mientras que la zona DNS vive en el panel de control, que es un sistema aparte — desde adentro del servidor no hay nada que editar. Reparto final: **dos registros a mano en hPanel**, y todo lo demás (el `wp-config.php` por SSH, los dominios en Vercel por CLI, y todas las verificaciones) por agente. Sin backup por API, el respaldo son dos cosas complementarias: captura del editor de zona completo, y un volcado por consulta DNS (solo lectura, sin credenciales) que es contra lo que se compara después del switch.
    - **Export real de la zona (27-ago). Tres hallazgos que cambian el plan:**
      1. **No hace falta esperar 24 h.** `www` y el apex **ya están en TTL 300** (los pone el propio CDN de Hostinger; las respuestas salen con TTL 60). Era el único paso del runbook con reloj, y ya estaba cumplido.
      2. **Los CAA no bloquean.** La zona tiene 12 registros `CAA`, que son una lista blanca de qué autoridades pueden emitir certificados para el dominio. Si la de Vercel no estuviera, el SSL no se emitiría después del switch y el sitio quedaría inaccesible por HTTPS sin un error obvio que apunte a la causa. Incluye `letsencrypt.org` y `pki.goog`, en `issue` y en `issuewild` — las dos que usa Vercel. Verificado, no supuesto.
      3. **El apex no es un registro A: es un `ALIAS`** al CDN, y `www` es un `CNAME`. En los dos casos hay que **cambiar el tipo de registro**, no editar un valor.
      La zona la sirven `athena`/`apollo.dns-parking.com`, o sea Hostinger — eso cierra el paso A0: NIC y TAD no se tocan en todo el cutover. Quedó además la lista explícita de lo que **no** se toca: los 5 MX de Google, el SPF, `_dmarc`, `resend._domainkey` (romperlo deja el formulario de contacto sin funcionar), el subdominio `send` (rebotes de Resend vía Amazon SES) y `wp`.
    - **Lo que falta, y en qué orden:** adjuntar el apex en Vercel → desactivar el CDN y volver a exportar la zona → `wp-config.php` con `WP_HOME`/`WP_SITEURL` → mover `www` **y** el apex a `76.76.21.21` → smoke test → E (webhook, Search Console, TTL de vuelta, HSTS, revocar credenciales).
    - **Obstáculo confirmado para la fase C, medido por DNS:** `www.usinadejusticia.org.ar` hoy es un CNAME a `www.usinadejusticia.org.ar.cdn.hstgr.net` (147.79.72.184 / 147.79.79.145) y el apex resuelve a 88.223.87.33 / 147.79.79.206 — es decir, **los dos están detrás del CDN de Hostinger**. Hay que desactivar el CDN para `www` **antes** de cambiar el registro, o Hostinger vuelve a poner su propio CNAME y pisa el cambio.
11. ~~**Webhook de revalidación instantánea (plugin `usina-headless`)**~~ — ✅ **RESUELTO (27-ago)**. Instalado, configurado y probado en producción: al editar un post, el cambio se ve en el sitio **al instante** en vez de esperar hasta 5 minutos. Endpoint `https://www.usinadejusticia.org.ar/api/revalidate`, con secreto compartido. Detalle de la configuración: la variable `REVALIDATE_SECRET` que ya existía en Vercel estaba marcada como sensible y **su valor no se puede volver a leer** —que es el comportamiento correcto—, así que se generó una clave nueva, se cargó en Production y Preview, se redesplegó, y recién entonces se cargó en WordPress. El orden importa: al revés, la prueba de conexión falla aunque todo esté bien. El plugin avisa por tres rutas al guardar (`/`, `/noticias` y la nota), solo para entradas publicadas —ni borradores ni páginas— y nunca bloquea el guardado del post si el aviso falla: en el peor caso se vuelve a la demora normal de ISR. **Registro de cómo estaba antes:** Hoy el contenido nuevo publicado en WordPress se refleja solo en el sitio nuevo dentro de una ventana de hasta 5 minutos (ISR, `revalidate = 300`), sin que nadie tenga que hacer nada — funciona, pero no es instantáneo. El plugin completo ya está escrito en `wp-plugin/usina-headless/` (un solo archivo PHP, sin dependencias, verificado con `php -l`) — al publicar/editar un post, le avisa al endpoint `/api/revalidate` (que ya existía) con el secreto compartido y las rutas que cambiaron, sin bloquear nunca el guardado del post si algo falla. Pantalla de ajustes propia (Ajustes → Usina Headless) con URL del endpoint, secreto (o una constante en `wp-config.php`, más segura) y un botón "Probar conexión". **Lo que falta es 100% de Emanuel, no de código**: subir la carpeta al WordPress real (FTP/SFTP o subida de .zip en wp-admin) y activarla — necesita acceso al hosting que el agente no tiene. Instrucciones completas en `wp-plugin/usina-headless/README.md`. No bloquea nada técnicamente: el sitio funciona igual sin él, solo con 5 minutos de demora en vez de segundos.
